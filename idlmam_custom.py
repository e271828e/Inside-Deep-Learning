...
def run_epoch( model, optimizer, data_loader, loss_func, device, results, score_funcs, prefix="", desc=None, disable_tqdm=False,
    ...
    for inputs, labels in tqdm(
        data_loader, desc=desc, leave=False, disable=disable_tqdm
    ):
...
class LastTimeStep(nn.Module):
    ...
    def forward(self, input):
        ...
        batch_size = last_step.shape[
            1
        ]  # per docs, shape is: '(num_layers * num_directions, batch, hidden_size)'
...
class AdditiveAttentionScore(nn.Module):
    ...
    def forward(self, states, context):
        ...
        state_context_combined = torch.cat(
            (states, context), dim=2
        )  # (B, T, D) + (B, T, D)  -> (B, T, 2*D)
...
class DotScore(nn.Module):
    ...
    def forward(self, states, context):
        ...
        scores = torch.bmm(states, context.unsqueeze(2)) / np.sqrt(
            D
        )  # (B, T, D) -> (B, T, 1)
        return scores
